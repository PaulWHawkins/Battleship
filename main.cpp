...
{
    //Game g(10, 10);
    //createPlayer("mediocre", "name1", g);
    //createPlayer("mediocre", "name2", g);
...
//        g.addShip(2, 'R', "rowboat");
    ...
    {
        Game g(10, 10);
        //addStandardShips(g); //problem: adding duplicate 
        addStandardShips(g);


        Player* p1 = createPlayer("good", "Good P1", g);
    ...
    else if (line[0] == '3')
    {
        ...
        int gWins = 0; int mWins = 0; 
        for (int i = 0; i < 100; i++) {
            Game g(10, 10);
            addStandardShips(g);
            Player* p1 = createPlayer("good", "Goody", g);
            Player* p2 = createPlayer("mediocre", "Bluto", g);
            cout << "This mini-game has one ship, a 2-segment rowboat." << endl;
            Player* winner = g.play(p1, p2);
            if (winner == p1) {
                gWins++;
            }
            else if (winner == p2)
        ...
        cerr << "Good player won " << gWins << "games and ";
        cerr << "bad player won " << mWins << "games";
